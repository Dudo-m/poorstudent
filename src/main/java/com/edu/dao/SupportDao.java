...
public interface SupportDao extends BaseMapper<Support> {
    ...
    //左连接分页,根据学生id查申请的资源搜索
    @Select("select t1.support_id,t2.*,t1.support_result from support t1 left join resource t2 on t1.resource_id = t2.resource_id where student_id = #{studentId} and t2.resource_name like CONCAT('%',#{resourceName},'%')")
    IPage<ResourceAndSupportVO> pageResourceVOBySearch(Page page, String studentId, String resourceName);
    ...
    @Select("select t1.support_id,t1.support_result,t2.*,t3.* from support t1 LEFT JOIN student t2 on t1.student_id=t2.student_id LEFT JOIN resource t3 on t1.resource_id=t3.resource_id where t3.resource_name like CONCAT('%',#{resourceName},'%')")
    IPage<ResourceStudentSupportVO> pageSupportVOBySearch(Page page, String resourceName);
